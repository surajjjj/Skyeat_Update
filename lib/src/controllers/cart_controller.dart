...
class CartController extends ControllerMVC {
  ...
  grandSummary() {
    ...
    if (currentCheckout.value.couponStatus == false) {
      ...
      tax = double.parse((pkgcharge+cal+(totalPrice * 5
      ) / 100).toStringAsFixed(
        ...
        if( currentCheckout.value.payment.method=='online') {
          ...
          if(currentCheckout.value.sub_total==0)
          {
            ...
            if(currentCheckout.value.sub_total>90) {
              ...
              String id=currentCheckout.value.shopId;
              // showToastMessage("we are not accepting orders below ₹90");

              String url = 'https://skynetitsolutions.com/skyeat_flutter/actions.php?action=send_notification_just&vendor_id=$id';
              final Map payload = {
                // "vendor_id":currentCheckout.value.shopId
              };
              try {
